...
export default function MatchesScreen({ route, navigation }: ScreenPropType<'Matches'>) {
  ...
  const thunkMatchData = (match: Match): AppThunk<Promise<void>> => async (dispatch) => {
  ...
  const onPressFactory = (m: Match) => () => {
    dispatch(thunkMatchData(m)).then(() => navigation.navigate('Match', { matchID: m.ID }))
  }
